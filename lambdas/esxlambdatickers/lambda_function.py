...
def get_ticker(ticker):
    ...
    return json.loads(http.request("GET", f"https://financialmodelingprep.com/api/v3/profile/{ticker}?apikey={API_KEY}").data.decode("utf8"))[0]


def get_clientip(clientip, tickers, cache=TTLCache(ttl=TIME_DELTA_CLIENT, timer=datetime.now, maxsize=MAX_SIZE_CLIENT_CACHE)):
    if clientip in cache: return cache[clientip]
    cache[clientip] = [get_ticker(ticker) for ticker in tickers[: MAX_TICKERS]]
    return cache[clientip]


def lambda_handler(event, context):
    tickers = [ticker.upper().strip() for ticker in event.get("queryStringParameters", {}).get("tickers", "").split(",") if ticker]
